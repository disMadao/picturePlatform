...
public class FileManager {
    ...
    public UploadPictureResult uploadPicture(MultipartFile multipartFile, String uploadPathPrefix) {
        ...
        String uploadFilename = String.format("%s_%s.%s", DateUtil.formatDate(new Date()), uuid,
                FileUtil.getSuffix(originFilename));//重做文件名称，原来的文件名称中可能有url不能解析的乱码，这样可以一定程度保证文件安全性
        String uploadPath = String.format("/%s/%s", uploadPathPrefix, uploadFilename);//最终上传路径，用户需要指定前缀，可能有不同文件夹
        File file = null;
        try {
            // 创建临时文件
